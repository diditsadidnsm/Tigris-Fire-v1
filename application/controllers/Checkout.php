...
class Checkout extends MY_Controller 
{
	...
	public function index($input = null)
	{
		...
		if (! $data['cart']) {
			$this->session->set_flashdata('warning', 'Tidak ada produk di dalam keranjang. Silahkan berbelanja terlebih dulu!');
			redirect(base_url('/cart'));
		}

		$data['input']	= $input ? $input : (object) $this->checkout->getDefaultValues();
		$data['title']	= 'Checkout Product Page';
		
		$this->load->view('layouts/_header', $data);
		$this->load->view('layouts/_navbar', $data);
		$this->load->view('pages/checkout/index', $data);
	...
	public function create()
	{
		...
		if ($order = $this->checkout->create($data)) {
			...
			$this->load->view('layouts/_header', $data);
		    $this->load->view('layouts/_navbar', $data);
		    $this->load->view('pages/checkout/success', $data);
		    $this->load->view('layouts/_footer');
		} else {
